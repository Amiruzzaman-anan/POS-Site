...
//    $conn = new mysqli("localhost", "aurexaac_buja", "bu!ja4#20,38?kd", "aurexaac_anan") or die("cannot connect database");
    ...
    $strdate = strval($date);
    //echo $strdate;
    $sql = "SELECT *,COUNT(*), SUM(`total`) AS asb FROM invoice WHERE month = '$strdate' GROUP BY `uid`";
    $result = mysqli_query($conn, $sql) or die("cannot execute query");
    $sql2 = "SELECT SUM(`total`) AS sum FROM `invoice` WHERE `month`='$strdate';";
    ...
    $sqldc = "SELECT SUM(`discount`) AS discsum FROM orders WHERE `month` = '$strdate';";
    ...
    $sqlprofit = "SELECT SUM(`total_profit`) AS prosum FROM invoice WHERE `month` = '$strdate';";
