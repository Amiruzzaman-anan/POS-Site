...
//    $conn = new mysqli("localhost", "aurexaac_buja", "bu!ja4#20,38?kd", "aurexaac_anan") or die("cannot connect database");
    ...
    $strdate = strval($date);
    // echo $strdate;
    $sql = "SELECT *,COUNT(*), SUM(`total`) AS asb FROM invoice WHERE `uid` = '$strdate' GROUP BY `uid`";
    $result = mysqli_query($conn, $sql) or die("cannot execute query");
    $sql2 = "SELECT SUM(`total`) AS sum FROM `invoice` WHERE `uid`='$strdate';";
    ...
    $sqldc = "SELECT SUM(`discount`) AS discsum FROM orders WHERE `order_id` = '$strdate';";
    ...
    $sqlprofit = "SELECT SUM(`profit`) AS prosum FROM orders WHERE `order_id` = '$strdate';";
        ...
        <div class="container-fluid">
          ...
          <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav mx-auto mb-2 mb-lg-0">
              ...
              <li class="nav-item">
                <a class="nav-link" href="product_user.php">Product</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="displayajex2.php">Company</a>
              </li>
...
<div class="container row">
  ...
  <div class="col">
    <level class="fw-bold">Search by date</level>
    <form method="post" action="datebysearch2.php" class="row g-3 mb-5">
      <input type="date" class="w-25 form-control" name="search">
      <button type="submit" class="ms-4 w-25 btn btn-outline-primary"> Search</button>
    </form>
  </div>
  <div class="col">
    <level class="fw-bold mb-3">Search By Month</level>
    <form method="post" action="monthbysearch.php" class="row g-3 mb-5">
        <select class="form-select w-25 form-control " name="search">
  ...
  <div class="col">
    ...
    <form method="post" action="yearbysearch.php" class="row g-3 mb-5">
        <input type="text" class="w-25 form-control" name="search">
        <button type="submit" class="ms-4 w-25 btn btn-outline-primary"> Search</button>
    </form>
  </div>
  <div class="col">
